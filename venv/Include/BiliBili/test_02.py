...
def get_play_list(start_url, cid, quality):
    ...
    headers = {
        'Referer': 'https://www.bilibili.com/video/av75854735',     # 注意加上referer
...
def down_video(video_list, title, start_url, page):
    ...
    currentVideoPath = os.path.join(sys.path[0], 'G:/shiyue/bilibili_video', title)  # 当前目录作为下载目录
    ...
    for i in video_list:
        ...
        # 请求头
        opener.addheaders = [
            ('Host', 'dataflow.biliapi.com'),  #注意修改host,不用也行
            ('User-Agent', 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.13; rv:56.0) Gecko/20100101 Firefox/56.0'),
            ('Accept', '*/*'),
            ('Accept-Language', 'en-US,en;q=0.5'),
            ('Accept-Encoding', 'gzip, deflate, br'),
            ('Range', 'bytes=0-'),  # Range 的值要为 bytes=0- 才能下载完整视频
            ('Referer', 'https://www.bilibili.com/video/av75854735'),  # 注意修改referer,必须要加的!
...
def combine_video(title_list):
    video_path = os.path.join(sys.path[0], 'G:/shiyue/bilibili_video')  # 下载目录
